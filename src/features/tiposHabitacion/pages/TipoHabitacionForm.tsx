...
const TipoHabitacionForm: React.FC = () => {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const { tiposHabitacion, createTipoHabitacion, updateTipoHabitacion, fetchTiposHabitacion } =
  ...
  // Fetch tipos de habitación once on mount
  useEffect(() => {
  ...
  useEffect(() => {
    if (id && tiposHabitacion.length) {
      const tipo = tiposHabitacion.find((t) => t.id === Number(id));
      if (tipo) {
        setForm({
          nombre: tipo.nombre,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (id) {
        await updateTipoHabitacion(Number(id), form);
      } else {
        await createTipoHabitacion(form);
      }
      navigate("/tipos-habitacion");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 p-6">
      <div className="max-w-2xl mx-auto">
        {/* Back Button */}
        <button
          onClick={() => navigate("/tipos-habitacion")}
        ...
        <div className="bg-slate-800/50 backdrop-blur-sm border border-slate-700 rounded-lg p-8">
          <h1 className="text-3xl font-bold text-white mb-2">
            {id ? "Editar Tipo de Habitación" : "Crear Tipo de Habitación"}
          </h1>
          <p className="text-slate-400 mb-8">
            {id ? "Modifica los detalles del tipo de habitación" : "Añade un nuevo tipo de habitación al sistema"}
          </p>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              <input
                ...
                placeholder="Ej: Habitación Simple"
                required
            ...
            <div>
              ...
              <textarea
                ...
                onChange={handleChange}
                placeholder="Ej: Habitación con una sola cama"
                rows={4}
