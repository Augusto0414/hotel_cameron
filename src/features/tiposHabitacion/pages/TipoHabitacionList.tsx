...
import { useTiposHabitacionStore } from "../store/useTiposHabitacionStore";

const TipoHabitacionList: React.FC = () => {
  const { tiposHabitacion, status, errorMessage, fetchTiposHabitacion, deleteTipoHabitacion } =
    useTiposHabitacionStore();
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 p-6">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            <h1 className="text-4xl font-bold text-white mb-2">Tipos de Habitación</h1>
            <p className="text-slate-400">Gestiona los tipos de habitaciones disponibles</p>
          </div>
          <Link
            to="/tipos-habitacion/nuevo"
        ...
        <div className="bg-slate-800/50 backdrop-blur-sm border border-slate-700 rounded-lg overflow-hidden">
          {tiposHabitacion.length === 0 ? (
            <div className="p-12 text-center">
              <p className="text-slate-400 text-lg">No hay tipos de habitación registrados</p>
              <Link
                to="/tipos-habitacion/nuevo"
              ...
              <table className="w-full">
                ...
                <tbody>
                  {tiposHabitacion.map((tipo) => (
                    <tr key={tipo.id} className="border-b border-slate-700 hover:bg-slate-700/30 transition-colors">
                      <td className="px-6 py-4 text-white font-medium">{tipo.nombre}</td>
                      <td className="px-6 py-4 text-slate-300">{tipo.descripcion || "—"}</td>
                      <td className="px-6 py-4 text-right">
                        <div className="flex justify-end gap-2">
                          <Link
                            to={`/tipos-habitacion/${tipo.id}`}
...
export default TipoHabitacionList;
